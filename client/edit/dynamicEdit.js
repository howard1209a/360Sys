...
function dynamicEditClass() {
  ...
  let isVideoRunning =
    $scope.buffer_empty_flag.filter((empty) => empty == true).length > 0
      ? false
      : true;
  ...
  if (ENABLE_EDIT && isVideoRunning && editEditInfoJSON) {
    ...
    if (CvpRadians != undefined) {
      ...
      if (
        editEditInfoJSON[next_edit] &&
        editEditInfoJSON[next_edit]["type"] == GRADUAL_EDIT
      ) {
        ...
        let currentFrameEdit = editEditInfoJSON[next_edit]
          ? editEditInfoJSON[next_edit]["frame"]
          : 0;
        let frameStartGradualRotation = Math.ceil(
          currentFrameEdit - EDIT_TIME * 2 * frameRate
        );
        let frameStopGradualRotation = Math.ceil(
          currentFrameEdit + EDIT_TIME * 0.5 * frameRate
        );
        let frameStartFade = Math.ceil(
          frameStopGradualRotation - (EDIT_TIME + FADE_TIME) * 2 * frameRate
        );
        ...
        if (currentFrame >= frameStartGradualRotation && enableRotation) {
          ...
          let predictedViewport = $scope.predict_center_viewport(
            currentFrameEdit - currentFrameEdit
          );
          let predict_center_viewport_x = predictedViewport[0];

          [dist_nearest_roi_gradual, direction] = getNearestRegionOfInterest(
            predict_center_viewport_x
          );
        ...
        if (isRotating) {
          ...
          if (direction > 0) {
            ...
            quaternionFace.setFromAxisAngle(
              new THREE.Vector3(0, 1, 0),
              ROTATION_SPEED
            );
            ...
            for (var face in camera_reference.faceStructure) {
              for (
                var position = 0;
                position < camera_reference.faceStructure[face].length;
                position++
              ) {
                camera_reference.faceStructure[face][position].applyQuaternion(
                  quaternionFace
                );
              }
            }
      ...
      if (
        editEditInfoJSON[next_edit] &&
        editEditInfoJSON[next_edit]["type"] == INSTANT_EDIT
      ) {
        let currentFrameEdit = editEditInfoJSON[next_edit]
          ? editEditInfoJSON[next_edit]["frame"]
          : 0;
...
function fireRotation(roi_radians) {
  ...
  console.log(
    "BEFORE camera_reference.faceStructure",
    camera_reference.faceStructure
  );
  const quaternionFace = new THREE.Quaternion();
  ...
  console.log(
    "AFTER camera_reference.faceStructure",
    camera_reference.faceStructure
  );
  camera_reference.object3D.quaternion.multiply(quaternion);
  sphere_reference.object3D.quaternion.multiply(quaternion);
...
function handleSnapCut(CvpXRadians) {
  ...
  [abs_dist_nearest_roi, dist_nearest_roi, index_dist_nearest_roi] =
    getNearestRegionOfInterest(CvpXRadians);
...
function getNearestRegionOfInterest(CvpXRadians) {
  ...
  for (let i in editEditInfoJSON[next_edit]["region_of_interest"]) {
    let ROIXRadians = convert_normalized_to_radians(
      editEditInfoJSON[next_edit]["region_of_interest"][i]["ROI_theta"]
    );

    let CvpXOpositeRadians =
      CvpXRadians > 0 ? CvpXRadians - Math.PI : CvpXRadians + Math.PI;

    let curr_dist_roi = getSphereRotation(
      CvpXRadians,
      CvpXOpositeRadians,
      ROIXRadians
    );
