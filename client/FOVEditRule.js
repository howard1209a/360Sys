...
function FOVEditRuleClass() {
  ...
  let DashManifestModel =
    factory.getSingletonFactoryByName("DashManifestModel");
  ...
  function getMaxIndex(rulesContext) {
    ...
    let current = abrController.getQualityFor(
      mediaType,
      streamController.getActiveStreamInfo().id
    );
    ...
    let segmentWithEdit =
      nextEditedFrame && Math.ceil(nextEditedFrame / frameRate);
    // Segment being displayed on video
    let currentSegmentPlayback =
      currentFrame && Math.ceil(currentFrame / frameRate);
    ...
    while (i >= 0 && count < 5) {
      ...
      if (
        ...
        let _totalTime =
          (currentRequest._tfinish.getTime() -
            currentRequest.trequest.getTime()) /
          1000;

        let _downloadTime =
          (currentRequest._tfinish.getTime() -
            currentRequest.tresponse.getTime()) /
          1000;
    ...
    calculatedBandwidth = latencyInBandwidth
      ? totalBytesLength / totalTime
      : totalBytesLength / downloadTime;
    ...
    if (predictWindow > 1) {
      // RIDGE REGRESSION
      predictedViewport = $scope.predict_center_viewport(
        predictWindow * frameRate,
        (ridge = true)
      );
    } else {
      // LINEAR REGRESSION
    ...
    if (currentSegmentNumber == segmentWithEdit) {
      let result = getNearestRegionOfInterest(
      ...
      if (result) {
        ...
        let nearest_region_of_interest =
          editInfo["region_of_interest"][index_dist_nearest_roi]["ROI_theta"];
        let roi_x_radians = convert_normalized_to_radians(
          nearest_region_of_interest
        );
  ...
  function computedQuality(
    ...
    let [isFaceVisible, percentageVisibleFace] = isFaceVisibleOnVP(
      info,
      predicted_visible_faces
    );

    if (isFaceVisible) {
      // console.log("FACE ", info.face, "IS VISIBLE AND ITS PERCENTAGE IS ", percentageVisibleFace);

      if (percentageVisibleFace < 0.1) {
        switchRequest.quality = maxQuality - 1;
        switchRequest.reason =
          "Face is slightly visible. Getting one quality lower than the max quality possible";
      } else {
        switchRequest.quality = maxQuality;
        switchRequest.reason =
          "Face is visible. Selecting high quality possible.";
      }
      ...
      switchRequest.reason =
        "Face is not visible. Selecting the lowest quality";
  ...
  function getNearestRegionOfInterest(CvpXRadians, editInfo) {
    ...
    for (let i in editInfo["region_of_interest"]) {
      let ROIXRadians = convert_normalized_to_radians(
        editInfo["region_of_interest"][i]["ROI_theta"]
      );
